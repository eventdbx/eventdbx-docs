...
- For zero-trust networks, layer Noise with WireGuard, IPSec, or hardware VPNs if regulators demand multiple independent controls.

Noise is the default transport security layer for EventDBX traffic. Reach for additional tunnels only when compliance or defense-in-depth policies call for a second encryption boundary.
