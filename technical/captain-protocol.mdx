...
Captain is EventDBX’s replication protocol. It runs over TCP with a baked-in Noise_XX handshake, multiplexes multiple streams, and keeps latency low even when nodes are far apart.

## Handshake

1. Client completes the Noise handshake and then sends a `HELLO` frame with domain, token fingerprint, and protocol version.
