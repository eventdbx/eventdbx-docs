...
Each plugin instance can request just the slice it needs. When you run `dbx plugin config ...`, pass `--payload <mode>`:

| Mode             | Includes                                      | Typical use case                  |
| ---------------- | --------------------------------------------- | --------------------------------- |
| `all`            | Event document, materialised state, and schema (default) | Rich projections or migrations    |
