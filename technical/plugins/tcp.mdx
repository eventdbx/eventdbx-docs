...
`noise_profile` references the shared Noise settings you defined under `[noise.*]`. Leave it unset only when the link already runs inside another encrypted tunnel (WireGuard, SSH, etc.).
...
When you skip the built-in Noise handshake, wrap the connection:
...
Alert on reconnect storms—they typically mean a firewall or consumer outage needs attention.
