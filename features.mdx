...
- **Tenant-level quotas** – `dbx tenant quota set` enforces per-tenant storage ceilings, `tenant list` reports live `quota_mb`/`usage_mb` counters, and `tenant quota recalc` rescans SSTs after restores so noisy neighbours cannot starve other domains.
