...
- **Tenant-aware guardrails** – `dbx tenant quota set`/`clear` enforce per-tenant storage ceilings, and `dbx tenant list` reports live usage so noisy neighbours cannot starve shared clusters.
