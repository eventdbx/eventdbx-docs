...
Every SDK exposes the same primitives—connect, authenticate, create aggregates, append events, and stream history. Pick the language that matches your service stack and follow the quickstart below.
...
with EventDBXClient(token="<token>", tenant_id="<tenant_id>", no_noise=False) as client:
    ...
    created = client.create(
        ...
        payload_json=json.dumps(
            {"first_name": "Ari", "last_name": "Diaz", "email": "ari@example.com"}
        ),
    ...
    updated = client.apply(
        ...
        payload_json=json.dumps({"email": "ari+alerts@example.com"}),
    )

    # List aggregates or fetch events for a specific aggregate
    aggregates = client.list(take=50, type="person")
    ...
    patched = client.patch(
        ...
        aggregate_id="p-110",
        event_type="person_registered",
        patches=[{"op": "replace", "path": "/last_name", "value": "Diaz-Garcia"}],
    )
...
- Synchronous TCP client with Noise security by default and an opt-out `no_noise` flag for lab setups.
- Built-in retry with exponential backoff and verbose/non-verbose mutation responses.
- Publish target routing plus archive/restore, JSON Patch, snapshots, and schema/tenant admin helpers.
...
client.aggregate("person")
      .apply("p-110", "person_email_updated", Map.of("email", "jaya@example.com"));
```
...
$client->aggregate('person')->apply('p-110', 'person_email_updated', [
...
await client.Aggregates["person"]
    .ApplyAsync("p-110", "person_email_updated", new {
...
err := cli.Aggregate("person").Apply(ctx, "p-110", "person_email_updated", eventdbx.Event{
    Payload: map[string]any{"email": "person@example.com"},
...
async fn main() -> eventdbx_client::Result<()> {
    ...
    let append = AppendEventRequest::new(
        ...
        "p-110",
        "person_registered",
        json!({ "first_name": "Nia", "last_name": "Patel" }),
    );
