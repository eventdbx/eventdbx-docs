...
Packages are published to GitHub Packages (`maven.pkg.github.com/eventdbx/eventdbx-java`). Add the repository and dependency to your build.
...
<Note>
  ...
  (`native/snownoise`). Run `cargo build --release --manifest-path
  native/snownoise/Cargo.toml` and ensure the built library is on
