...
When sorting by `created_at` or `updated_at`, you can pass shorthand timestamp cursors (`cursor="ts:<aggregate_type>:<aggregate_id>"`) alongside the timestamp sort and the server will expand them into full tokens (`ts:<field>:<order>:<scope>:<timestamp_ms>:...`) before paging.
